...
def test_import_google_photos_selection_feeds_photo_autofill_without_photo_date_fallback(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    assert response["normalized_payload"]["service_date"] == ""
    ...
    assert google_diagnostics["location_status"] == "from_ocr"
    assert google_diagnostics["service_date_policy"].startswith("Picker createTime")
