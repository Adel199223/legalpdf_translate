...
The shared implementation is `word_automation.py` with `word_pdf_control.py`, `word_pdf_artifacts.py`, `word_pdf_script.py` and the retained-handle launcher in `word_process_start.py`. Production export is for the app-generated honorarios document, not the translation. Preserve the editable original and any previous good PDF while producing a unique staged output; parse every PDF page and promote only verified fresh output. A canary must also contain its expected text. Separately authorized internal Word-PDF layout proof does not change translation deliverables from DOCX.
...
- Launch the resolved Word executable directly and hidden with the single `/w` switch, retaining its process handle. Require all seven startup checks before document opening: a returned process, running status, a new PID, matching session, `WINWORD` name, start time not before launch and matching executable. Bind that process's document window and verify ownership before changing visibility/security or opening the staged document. Do not attach through `GetActiveObject`, accept a substituted process, or equate COM construction with ownership.
...
The user-approved v18 footer pilot subsequently passed one guarded native export and one-page visual review. The [completed integration plan](../exec_plans/completed/2026-09-09_footer_spacing_integration.md) records only the accepted formatter/native subset in a clean tree; 615 focused tests, 2,492 full-suite tests and serial full validation passed. Publication, merge after successful checks and app verification are explicitly approved; actual GitHub/main outcomes must be verified separately. That approval does not authorize another native attempt, recovery replay or host-setting change. Keep startup diagnostics content-free, retain the original journal during bounded recovery, and require separate authority for any future native operation. Pilot success is not a full-case/model-quality claim.
