...
The user-approved v18 footer pilot subsequently passed one guarded native export and one-page visual review. The [current integration plan](../exec_plans/active/2026-09-09_footer_spacing_integration.md) carries only the accepted formatter/native subset into a clean tree; 615 focused tests, 2,492 full-suite tests and serial full validation passed, while publication/merge remain pending. That stage does not authorize another native attempt, recovery replay or host-setting change. Keep startup diagnostics content-free, retain the original journal during bounded recovery, and require separate authority for any future native operation. Pilot success is not a full-case/model-quality claim.
