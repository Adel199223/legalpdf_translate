...
## Unpublished structured-activation prerequisites (2026-09-10)

The active [ordinary-use ExecPlan](docs/assistant/exec_plans/active/2026-09-10_structured_translation_ordinary_use.md)
tracks Stage 2 in the isolated `legalpdf_translate_structured_activation` worktree,
branch `feat/structured-translation-activation`, based on PR #295 merge
`4780a2e16c32adf7af7479656f0b5fafdff83224`. These changes are not deployed or merged.
Canonical `main` and its uncommitted handoff documentation remain untouched.
The first 37-file Stage 2 candidate passed 3,508 full-suite tests and serial
validation. Stage 3 preparation then identified pricing and acceptance-continuity
prerequisites. The user authorized corrective offline Stage 2; its new 46-file
candidate passed 385 targeted tests, 3,624 full tests, independent review and serial
full validation (known successful Dart fallbacks). Stop at fresh `NEXT_STAGE_3`
for acceptance preparation only. Historical evidence remains preserved.
Real AR/FR/EN legal/layout/cost acceptance and activation remain open.

The candidate keeps `legacy_text_v1`, GPT-5.2/high, OCR policy and formatting
defaults. Browser resume forces `resume=True`; workflow settings come from the
caller's workspace. Configured provider clients are cloned without resolving a
different key. Automatic translation authentication checks local credentials;
the first accounted work call proves remote authorization. Automatic post-run
save-row suggestions use local text only; explicit autofill remains available.

Each real translation/OCR/correction/auth dispatch has a unique accounting ticket.
Translation runs persist a per-generation journal referenced by the checkpoint;
resume cannot silently replace damaged/missing accounting evidence. Standalone
diagnostics retain process-lifetime records, not a lifetime cost claim. Old
unmetered history is incomplete. Reasoning is included in output totals, cached
input is separately priced, and missing usage/model/prices never mean zero cost.
Ordinary workflows now receive a caller-owned immutable accounting policy. Its
dated public GPT-5.2/default-tier/USD rates apply only to the verified public API
endpoint and matching actual model/tier; other or missing billing identities stay
incomplete. Bundled reference rates expire for new runs after 30 days (a local
conservative policy, not a provider guarantee); saved journals retain their exact
catalog and never silently reprice. Legacy forecast rates are not billing proof.
The default no-cap/WARN policy and omitted/auto request tier remain unchanged.
An explicit BLOCK cap requires durable reservations and verified request bounds;
auto-tier hard caps require a finite verified worst-case model/tier set. Unknown
scope, rates or bounds stop before dispatch. Gemini hard-budget billing scope is
not yet verified. The original USD10 acceptance ledger is separate from ordinary
product budgets and is never reset or replaced.

Acceptance-only v2 amendments bind the original ledger, stable full-case context,
approved page/purpose/request subset and immutable pricing/budget controls. Pilot
expansion reuses committed pages; private durable responses and pending correction
evidence permit at most one separately approved correction without rebuying the
primary. Actual source bytes, executing package membership, effective config and
preferences, pending evidence, endpoint and final request are checked before
reservation and immediately before contact. This is an internal serial runner
seam, not a new browser/CLI option or self-approval mechanism. Preflight is
read-only and does not bind runtime authority or dispatch.

Offline browser construction uses an explicit service bundle with truthful
`not_evaluated` capability results. Isolated workflow execution additionally
injects environment and OCR construction boundaries; shadow mode alone is not
proof of isolation. Real AR/FR/EN legal quality, full Word layout, dated pricing,
complete measured cost and fresh paid/native permissions remain Stage 3 gates.
No default activation, model promotion, Word/Gmail operation or publication is
authorized by this implementation. See the plan for current validation status.
...
Ordinary-launch translation still returns page TXT by default, not source-associated translated blocks. New TXT runs receive conservative readable typography/RTL/PAGE defaults, but the writer never invents source/target alignment from matching paragraph counts or numbers. Full source-aware layout is available for retained, validated structured runs through local rebuilds and the merged opt-in integration below. Durable protocol activation and model-quality promotion remain separate. Rebuilds reuse saved translations, preserve historical usage/review evidence, and do not construct API/OCR clients. Source-map rendered page counts remain unknown until measured in Word.

Merged PR #295 connects fresh runs to this formatter through `TranslationWorkflow(translation_protocol="legal_blocks_v2")`, or the internal `LEGALPDF_TRANSLATION_PROTOCOL` environment selection. Ordinary launches still default to `legacy_text_v1`; the verification process's session-only flag is not durable production activation or a new browser/CLI submitted value. Saved checkpoint identity wins for an implicit resume; explicit conflicts fail before writes/authentication. Browser service/routes, CLI/queue and Qt callers share the same coordinator. See [the completed implementation/publication closeout](docs/assistant/exec_plans/completed/2026-09-09_structured_new_translations.md) for measured evidence. Canonical `main` at `C:/Users/FA507/.codex/legalpdf_translate`, merge SHA `4780a2e16c32adf7af7479656f0b5fafdff83224`, is routine app authority. The old source branches were deleted and the task tree is retained detached; publication is no longer pending.

The opt-in implementation retains all selected digital source blocks and passive TSV evidence from the same winning local OCR pass. It adds no OCR pass/provider and does not replace OCR/model/effort routing. Exact source-token mismatch discards geometry, not the chosen text. API-only/plain OCR and uncertain column order remain layout-review-required. Neighbor context is bounded to 600 characters per side, only from selected direct-text pages proven authoritative; it cannot trigger extra OCR. Strict Responses JSON coverage is checked separately from refusal/incomplete status. At most one source-grounded correction follows a demonstrated block defect; unsuccessful/cancelled responses retain available usage and never become completed pages.
...
Stage 4 is complete: PR #295 merged at `4780a2e16c32adf7af7479656f0b5fafdff83224`. Final source-grapheme validator v5 passed 318 focused tests and 3,391 local full tests in 428.49s; independent review and serial full validation passed with the known successful Dart fallback. Final exact-head CI run `34411719376` passed both jobs (3,391 Windows tests); canonical verification passed 20 offline checks and a guarded UI/API-wiring smoke. All 46 runtime/test files matched the accepted publication digest; source branches were deleted and the completed task tree retained clean/detached. Smoke capability boundaries were explicitly not evaluated, not proof of real provider/auth/Gmail/native readiness. The original unsafe GET/bootstrap helper was not executed; shadow mode and a disabled bridge alone do not prevent Word/account side effects. Follow the isolation workflow before new checks. Ordinary protocol/model defaults remain unchanged (`legacy_text_v1`, checked-in `gpt-5.2`/high); no paid/native operation or durable activation was added by publication. Start the next Stage 1 controlled-activation assessment with [FRESH_CHAT_BRIEF.md](docs/assistant/FRESH_CHAT_BRIEF.md), preserving the local handoff docs, original budget, advanced Word journal and private artifacts.
