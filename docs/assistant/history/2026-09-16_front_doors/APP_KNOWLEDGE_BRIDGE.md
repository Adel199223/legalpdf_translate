...
Unpublished 2026-09-10 structured-activation prerequisites are tracked in the
[active ordinary-use plan](exec_plans/active/2026-09-10_structured_translation_ordinary_use.md)
and the canonical document's matching status section. The isolated candidate
adds resume/settings/bootstrap isolation, immutable tier-aware dispatch accounting
and separately approved acceptance continuation, but keeps
ordinary protocol/model defaults unchanged. Do not confuse that worktree with
canonical `main`, or offline validation with real legal/Word/cost acceptance.
...
- Fresh source-associated translation is merged as PR #295 at canonical `main@4780a2e16c32adf7af7479656f0b5fafdff83224`, still opt-in rather than the ordinary-launch default. Follow canonical `Output and Run Artifacts`, [the completed closeout](exec_plans/completed/2026-09-09_structured_new_translations.md) and [FRESH_CHAT_BRIEF.md](FRESH_CHAT_BRIEF.md). Both source branches were deleted; the task tree is retained clean/detached. Legacy resume, DOCX-only delivery and manual-review gates remain intact.
- Bounded synthetic native/human layout acceptance, local 3,391-test/full validation, final exact-head CI (both jobs) and canonical 20-test/UI-wiring verification are complete. Ordinary launches retain `legacy_text_v1`; model defaults were not promoted. Real provider/native/Gmail health, full-reference legal/layout acceptance and total cost savings are not established by the guarded smoke. Native authority is consumed; preserve the advanced journal, budget, saved artifacts and local handoff docs. The next task is Stage 1 controlled-activation assessment, not repeating publication or native checks.
