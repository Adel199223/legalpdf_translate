...
- Fresh source-associated translation integration is an opt-in publication candidate, not an ordinary-launch default; follow canonical `Output and Run Artifacts` and [the implementation/publication closeout](exec_plans/completed/2026-09-09_structured_new_translations.md). Canonical `main` remains routine app authority; the isolated `codex/structured-new-translations` branch is the publication candidate until merge is verified. Legacy resume, DOCX-only delivery and manual-review gates remain intact.
- Clock corrective Stage 3 passed the explicitly approved single Arabic Word check in 6.183 seconds, with owned-process cleanup confirmed. Two full-page visual reviews and physical glyph coordinates confirm correct `09:30`, coherent names, header spacing and bottom footer on one synthetic A4 page. English/French reuse exact package-matched native evidence. The user approved the corrected layout with `looks right` on 2026-09-09. Native authority is consumed; preserve the advanced original journal, saved evidence and historical failures. Stage 4 publication/canonical verification is now explicitly authorized. Final source-grapheme validator v5 hardening is a two-file source/test delta: 318 focused tests passed in 7.39s, final full pytest passed 3,391 tests in 428.49s, and independent review is clear; serial release validation passed with exit code 0 and the known successful direct-Dart fallback. The earlier 46-file validation is historical. Require observed GitHub checks before merge; this branch closeout claims no CI, merge or post-merge result. Ordinary launches retain `legacy_text_v1`; a session-only verification flag is not durable activation. No new paid/native/model/host scope is authorized.
...
- Google Photos Interpretation import, OAuth/Picker setup, Picker troubleshooting, metadata provenance, and sanitized live-validation hygiene route first to `docs/assistant/features/GOOGLE_PHOTOS_INTERPRETATION_RUNBOOK.md`, then `APP_KNOWLEDGE.md` and `docs/assistant/VALIDATION.md`.
...
- Honorarios PDF ownership, bounded export, quarantine/manual recovery and the September native acceptance evidence route to `APP_KNOWLEDGE.md` and [the completed repair ExecPlan](exec_plans/completed/2026-09-07_honorarios_pdf_export_reliability.md). Use that historical evidence together with [the completed footer-spacing integration plan](exec_plans/completed/2026-09-09_footer_spacing_integration.md); the later footer/native release is included in the current task's PR #294 baseline.
...
- Canonical current-truth also includes the browser static frontend modularization boundary: coordinator modules keep state, API calls, routing, and side effects; focused UI modules own safe DOM rendering; and focused presentation modules derive labels, status, and card data.
- Canonical current-truth now includes Google Photos import for Interpretation requests only: Picker OAuth uses the minimal `photospicker.mediaitems.readonly` scope, Picker sessions are created/opened/polled/listed/cleaned up, selected image bytes feed the existing photo/OCR autofill path, and `Review Case Details` stays the review gate. Google Photos place/location remains unavailable from the Picker API; service city comes from OCR/document evidence or user selection. Photo date can prefill service date only as an editable fallback when OCR/legal text has no date.
...
- Canonical current-truth also includes the Windows-only Gmail intake bridge, native-host live-server preparation from the current canonical checkout, exact-message Gmail fetch/review, same-tab Gmail redirect into the browser workspace, fail-closed Gmail-page banner errors on rejected handoffs, sequential Save-to-Job-Log gating, one honorários DOCX plus sibling PDF attempt for the confirmed batch, and one threaded Gmail reply draft with no auto-send.
- Canonical current-truth also includes the March-April browser-hardening layer: validated native-host runtime selection, canonical no-console native-host EXE registration, shell-safe browser startup, per-click `handoff_session_id`, `source_gmail_url`, `bridge_context_posted` diagnostics, client-ready/asset-version proof for stale asset detection, and bundled browser-managed PDF preview/prepare instead of startup-time `PyMuPDF`.
...
- Canonical current-truth also includes the accepted same-tab Gmail recovery: a valid click redirects the current Gmail tab, posts bridge context after redirect commit, avoids `Pending load`/unavailable IDs, avoids repeated CMD/PseudoConsole churn, and exposes `Return to Gmail` using the captured source URL.
...
- The accepted separate honorarios PDF repair adds exact process/window/document ownership, staged DOCX/fresh-PDF verification, shared serialization and quarantine after uncertain cleanup. Initial browser/Qt export is one 45-second call with no automatic timeout retry; explicit Qt manual retry and existing payloads/signals remain unchanged. Eight real Word exports took approximately 6-8 seconds each, all six retained honorarios PDFs passed complete-page visual review, and the source DOCX plus tested open/unsaved Word state were preserved. The repair originated on `feat/honorarios-pdf-export` from `main@9afe1d05`; its acceptance evidence is historical, and the later footer/native release is included in PR #294. Translations remain DOCX, no model policy or live Gmail behavior was promoted, and the separate native Qt layout-test diagnostic remains documented.
...
- Canonical current-truth also includes the Gmail honorários local-court metadata rule: specific court-unit evidence such as `Juízo de Competência Genérica de Cuba`, address city `Cuba`, or `cuba.ministeriopublico@tribunais.org.pt` outranks generic comarca text such as `Comarca de Beja` for translation save seeds, Job Log rows, honorários output, and Gmail finalization while keeping the extracted court email unchanged.
