...
- Canonical current-truth also includes translation `run_report.md` as a first-class browser artifact: the completion drawer can generate or refresh `<run_dir>/run_report.md`, download it immediately once, and keep a persistent `Download Run Report` action afterward.
