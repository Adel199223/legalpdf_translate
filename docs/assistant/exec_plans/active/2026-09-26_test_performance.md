...
Implementation and local validation are complete. Keep this plan active until the new test-infrastructure change's publication/application scope is resolved. Canonical main remains the clean prior app build at `b35be1c`; no application code, saved defaults, ledger rows, paid operations or live acceptance changed. Prepare the tested branch for review and obtain any still-required publication authority under `AGENTS.md` before push/PR/merge/application. Actual hosted CI timing and success remain unverified until that workflow runs.
