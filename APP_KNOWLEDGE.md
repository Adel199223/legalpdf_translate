...
The five-request fixes were implemented from applied PR #307 baseline `2a6eb5962820b3c0f015dc6e296516ff2dac8049`. The user approved publication, canonical application and continuation of the remaining selected translations on 2026-09-26. Publication is in progress; exact PR/check/merge/application status belongs in `C:/Users/FA507/.codex/legalpdf_translate_private_benchmarks/five_oldest_requests_20260926_01/publication_01/publication_receipt.json` and the actual repository state. Do not treat the implementation baseline as a permanent current HEAD. [HANDOFF.md](docs/assistant/HANDOFF.md) owns current runtime evidence. The active [five-request workflow](docs/assistant/exec_plans/active/2026-09-26_five_oldest_translation_requests.md) has two reviewed unsent drafts; three requests remain unstarted. The first owned runtime is retired. No email was sent.

The reviewed fixes address three observed problems: stale word counts after DOCX edits, a legal-citation line mistaken for a footer, and split Arabic decimal currency values. Focused regressions and independent review passed; final combined Full passed 664 selected test executions in 812.809 seconds, with compilation and docs/hygiene checks. Product hashes remain frozen. Continue after the authorized publication/application lifecycle and actual-head successor verification, without routine reconsent for the remaining selected requests. Earlier outputs, ledger entries, saved defaults and the original USD10 limit remain preserved; the old FRlong hold is unresolved. These operational results do not certify all documents or legal accuracy.
...
The reviewed count correction refreshes a completed browser job's displayed count from its actual DOCX. Saving a current job reads that server-owned file again; Gmail confirmation stages the file first and counts those exact attachment bytes. Missing/unreadable content blocks saving, while the original job/report remains viewable with a warning. Calculated totals follow the new count; explicit different amounts and historical rows remain editable. This does not rewrite historical run summaries or already-finalized drafts. Check HANDOFF for publication status.
