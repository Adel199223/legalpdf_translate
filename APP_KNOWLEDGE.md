...
Current scoped integration: the user accepted the one-page Arabic footer pilot after the v18 Word export and visual review, then authorized `NEXT_STAGE_4`. The [footer-spacing integration ExecPlan](docs/assistant/exec_plans/active/2026-09-09_footer_spacing_integration.md) tracks the clean subset on `codex/footer-spacing-integration` from `main@ec406dac`. New-tree validation passed: 615 focused tests, 2,492 full-suite tests and serial `validate_dev.ps1 -Full`, including successful direct-Dart fallback. GitHub publication and merge remain pending separate approval; canonical `main` is unchanged. This acceptance covers the retained pilot, not a whole-document, language or model benchmark.
