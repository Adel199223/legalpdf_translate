...
function setClientHydrationMarker(status, { payload = null, reason = "", message = "" } = {}) {
  ...
  const nextBootstrappedAt = nextStatus === "ready"
    ...
    : null;
  const markerPayload = payload || appState.bootstrap?.normalized_payload;
  ...
  const marker = {
    ...
    gmailHandoffState: deriveClientGmailHandoffState({
      payload: markerPayload,
      workspaceId: appState.workspaceId,
    }),
    buildSha: currentBuildSha(),
    assetVersion: currentAssetVersion(),
    launchSessionId: deriveClientLaunchSessionId({ payload: markerPayload, href }),
    handoffSessionId: deriveClientHandoffSessionId({ payload: markerPayload, href }),
    launchSessionSchemaVersion: deriveClientLaunchSessionSchemaVersion({ payload: markerPayload, href }),
