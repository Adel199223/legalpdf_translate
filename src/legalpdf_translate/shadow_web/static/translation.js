...
} from "./translation_numeric_warning_presentation.js";
import {
...
export function deriveNumericMismatchWarning(job = translationState.currentJob, extra = null) {
  ...
  return deriveTranslationNumericMismatchWarning({
    job,
    extra,
    cachedWarning: jobId ? translationState.numericMismatchWarningsByJobId[jobId] : null,
  });
}

function blankNumericMismatchWarning(options = {}) {
  return blankTranslationNumericMismatchWarning(options);
}

export function renderNumericMismatchWarningInto(containerOrId, warning = blankTranslationNumericMismatchWarning()) {
  const container = typeof containerOrId === "string" ? qs(containerOrId) : containerOrId;
  if (!container) {
    return;
  }
  const normalized = warning?.visible ? warning : blankTranslationNumericMismatchWarning();
