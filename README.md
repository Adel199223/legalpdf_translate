...
Start with `-Quick` for core logic and browser-state feedback. The default command retains the browser/API tier; `-Full` retains the broader selected integration coverage and runs its formatting files in two isolated processes. Use `-Full -Workers 1` for sequential diagnosis. Neither selected tier replaces complete-repository CI coverage. In the local, unpublished test-performance work, Quick passed 87 tests with two DOM integration cases excluded only from that tier (10.72 seconds pytest; 16.202 seconds including the wrapper). Selected Full passed 659 tests in 838.559 seconds (13 minutes 58.56 seconds). The complete four-worker benchmark passed all 6,717 original cases plus 37 runner tests in 537.941 seconds (8 minutes 57.94 seconds), versus the 1,835.010-second baseline: 3.41x in one local comparison. The baseline overlapped short Quick/tool checks; no hosted-CI timing is established. See the [validation guide](docs/assistant/VALIDATION.md) for measured scope and evidence limits.
