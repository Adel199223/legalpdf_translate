...
set "LAUNCHER_SCRIPT=%REPO_ROOT%\tooling\launch_qt_build.py"

if not exist "%LAUNCHER_SCRIPT%" (
  echo [LegalPDF] Missing launcher helper:
...
"%PYTHON_EXE%" "%LAUNCHER_SCRIPT%" --worktree "%REPO_ROOT%" --allow-noncanonical
if errorlevel 1 (
  echo.
  echo [LegalPDF] App launch failed.
  pause
  exit /b 1
)
